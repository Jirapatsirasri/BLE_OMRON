...
*/
#include "Magellan_BC95_v4.h"
Magellan_BC95_v4 magel;          
...
void loop() {
  ...
  */
  ...
  data=magel.getConfig("zero",token);
  Serial.print("data ");
  Serial.println(data);

  String light=magel.getDelta("light",token);
  Serial.print("light SW ");
  Serial.println(light);

  String AC=magel.getDelta("AC",token);
  Serial.print("AC SW ");
  Serial.println(AC);

  
  if(AC.indexOf("40300")==-1)
  {
    String report_AC="{\"AC\":"+AC+"}";
    magel.report(report_AC,token);
  }
